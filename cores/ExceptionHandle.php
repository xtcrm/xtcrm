...
class ExceptionHandle extends Handle
{
    ...
    {
        return [
            'Access-Control-Allow-Origin'      => env('cors.allow_origin', '*'),
            'Access-Control-Allow-Credentials'  => 'true',
            'Access-Control-Max-Age'            => 1800,
            'Access-Control-Allow-Methods'      => 'GET, POST, PATCH, PUT, DELETE, OPTIONS',
            'Access-Control-Allow-Headers'      => 'Authorization, Content-Type, X-CSRF-TOKEN, X-Requested-With, Access-Token, storeId, platform',
        ];
    }
     ...
     * @return Json
     */
    private function withCorsHeaders(Json $response): Json
    {
    ...
    public function render($request, Throwable $e): Response
    {
        ...
        if ($e instanceof BaseException) {
            ...
            return $this->withCorsHeaders($this->output($extend));
        }
        // 系统运行的异常
        ...
        // 如果是debug模式, 输出调试信息
        if (is_debug()) {
            return $this->withCorsHeaders($this->outputDebug($e));
        }
        // 将运行异常写入日志
        $this->errorLog($e);
        return $this->withCorsHeaders($this->output());
    }
    ...
    private function errorLog(Throwable $e)
    {
        ...
        foreach (['access-token', 'authorization', 'cookie', 'x-csrf-token'] as $key) {
            if (isset($headers[$key])) $headers[$key] = '[REDACTED]';
        }
