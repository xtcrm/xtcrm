<template>
  <a-card :bordered="false">
    <a-page-header style="padding:0 0 16px 0" title="新增客户" @back="() => router.push('/crm/customer/index')" />
    <a-spin :spinning="loading">
      <a-form ref="formRef" :model="formState" :label-col="{ span: 3 }" :wrapper-col="{ span: 10 }">
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="行业" name="industry_id" :rules="[{ required: true, message: '请选择行业' }]">
              <a-select v-model:value="formState.industry_id" placeholder="选择行业" show-search :filter-option="filterOption" @change="(v) => setDictName('industry', v, 'industry')">
          ...
          <a-col :span="12">
            <a-form-item label="客户分组" name="group_id" :rules="[{ required: true, message: '请选择客户分组' }]">
              <a-select v-model:value="formState.group_id" placeholder="选择分组" show-search :filter-option="filterOption" @change="(v) => setDictName('customer_group', v, 'customer_group')">
          ...
          <a-col :span="12">
            <a-form-item label="客户等级" name="level_id" :rules="[{ required: true, message: '请选择客户等级' }]">
              <a-select v-model:value="formState.level_id" placeholder="选择等级" @change="(v) => setDictName('customer_level', v, 'level_name')">
          ...
          <a-col :span="12">
            <a-form-item label="客户来源" name="source_id" :rules="[{ required: true, message: '请选择客户来源' }]">
              <a-select v-model:value="formState.source_id" placeholder="选择来源" show-search :filter-option="filterOption" @change="(v) => setDictName('customer_source', v, 'source')">
          ...
          <a-col :span="12">
            <a-form-item label="信用额度" name="credit_limit">
              <a-input-number v-model:value="formState.credit_limit" :min="0" :precision="2" style="width:100%" /> 元
            </a-form-item>
          </a-col>
          <a-col :span="12">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="负责人" name="owner_user_id">
              <a-select v-model:value="formState.owner_user_id" placeholder="选择负责人（为空则放入公海）" allow-clear show-search :filter-option="filterOption" @change="onOwnerChange">
                <a-select-option v-for="u in storeUsers" :key="u.store_user_id" :value="u.store_user_id">{{ u.real_name || u.user_name }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="负责人部门" name="owner_department_id">
              <a-tree-select v-model:value="formState.owner_department_id" placeholder="选负责人后自动填入，也可手动选" allow-clear tree-default-expand-all :tree-data="deptTree" :dropdown-style="{ maxHeight: '300px', overflow: 'auto' }" />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-item label="备注" name="remark" :label-col="{ span: 1 }" :wrapper-col="{ span: 22 }">
              <a-textarea v-model:value="formState.remark" placeholder="备注信息" :rows="3" />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row>
          <a-col :span="24" style="text-align:center;padding-top:24px">
            ...
            <a-button style="margin-left:8px" @click="() => router.push('/crm/customer/index')">取消</a-button>
          </a-col>
        </a-row>
      </a-form>
    </a-spin>
...
import { add, checkNameDup } from '@/api/crm/customer'
...
const dictOptions = reactive({
  industry: [], customer_level: [], customer_source: [], customer_group: [],
})
...
const formState = reactive({
  ...
  owner_user_id: undefined, owner_department_id: undefined,
  remark: '',
})
...
function loadUsers() {
  getStoreUsers({}).then(res => {
    ...
    if (uid) formState.owner_user_id = uid
  })
}
...
function buildDeptTree(list, parentId = 0) {
  ...
  return list.filter(d => d.parent_id === parentId).map(d => ({
    title: d.department_name, value: d.id, key: d.id,
...
async function validateCustomerName(_rule, value) {
  ...
  try {
    const res = await checkNameDup(value, 0)
...
function handleSubmit() {
  formRef.value.validate().then((values) => {
    ...
    add(values).then(() => {
      ...
      router.push('/crm/customer/index')
    }).catch(() => { loading.value = false })
  }).catch(() => {})
}

onMounted(() => { loadDicts(); loadUsers(); loadDepts() })
</script>
